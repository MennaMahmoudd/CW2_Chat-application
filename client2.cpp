...
bool initialize();
void signUp();
void login(string& username); // Forward declaration for login
...
void showChatHistory(const string& username) {
    ifstream inFile(username + "_chat_history.txt");
    if (inFile.is_open()) {
        string line;
        while (getline(inFile, line)) {
            cout << line << endl;
...
void appendToChatHistory(const string& username, const string& message) {
    ofstream outFile(username + "_chat_history.txt", ios::app);
    if (outFile.is_open()) {
    ...
    while (true) {
        ...
        if (recvlength <= 0) {
            ...
            if (usernamePos != string::npos && messagePos != string::npos) {
                ...
                // Append the received message to chat history
...
int main() {
    ...
    switch (choice) {
        ...
        case 2:
            ...
            break;
        case 3:
            login(username); // Log in first
            showChatHistory(username); // Show chat history
            close(s);
            return 0; // End the program after showing history
        default:
